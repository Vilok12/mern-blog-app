...
authorapp.post('/article',verifytoken("AUTHOR"),checkauthor,async(req,res)=>{
    // get article 
...
authorapp.get('/article/:authid',verifytoken("AUTHOR"),checkauthor,async(req,res)=>{
    // get author id
...
authorapp.put('/article',verifytoken("AUTHOR"),checkauthor,async(req,res)=>{
...
authorapp.patch('/delarticle',verifytoken("AUTHOR"),checkauthor,async(req,res)=>{
    let {author,articleid}=req.body
    // find article
    let articleofdb=await articlemodel.findOne({_id:articleid,author:author})
    if(!articleofdb){
        res.status(403).json({message:"Forbidden, you can modify your own article only"})
    }
    // find articleand update
